...
- "Nick Kjær Kristoffersen <nkch@itu.dk>"
...
![Domain Model](Chirp/docs/diagrams/png/Domain%20Model.png)
...
3. To allow registering with GitHub `cd` into `src/Chirp.Web` and run the three following commands in your terminal:
    ...
    2. `dotnet user-secrets set "authentication_github_clientId" "Ov23li8tJVPjkWxP3PA0"`
    3. `dotnet user-secrets set "authentication_github_clientSecret" "dbb574deaac6b57c60ad2d322fd84b9caf22f83d"`

4. To ensure HTTPS `cd` into `src/Chirp.Web` and run the following in your terminal: `dotnet dev-certs https --trust`
6. To run the project `cd` into `src/Chirp.Web` and from here in the terminal run: `dotnet run`.
...
- [ ] "Process"
    - [x] "Build, test, release, and deployment"
    - [ ] "Team work"
    - [x] "How to make Chirp! work locally"
    - [x] "How to run test suite locally"
- [x] "Ethics"
    - [x] "License"
